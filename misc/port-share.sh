#!/bin/bash
### Start the ssh tunnel on the server, if it is not already up.
...
OPEN_TUNNEL="ssh -qfNT -R $PROXY_PORT:localhost:$SERVER_PORT $proxy_connect"
...
cronfile=/etc/cron.d/sshtunnel-$SERVER_PORT
if [ ! -f $cronfile ]; then
    script=$(readlink -f $0)
    cat <<EOF > $cronfile
